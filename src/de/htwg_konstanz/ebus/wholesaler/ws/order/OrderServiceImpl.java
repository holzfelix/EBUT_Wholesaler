    ...
    public OrderServiceResponse purchaseOrder(de.htwg_konstanz.ebus.wholesaler.ws.order.OrderServiceRequest request) throws OrderServiceFault {
        ...
        try {
         ...
         // build the Order Customer
            ...
            if (boCurrency != null) {
                if (!boCurrency.getCode().equalsIgnoreCase("EUR")) {
                    throw new OrderServiceFault("Die angegebene W�hrung wird nicht unterst�tzt! Dieses System unterst�tzt derzeit nur EUR als W�hrung!", boCurrency.getCode());
                }
            } else {
                throw new OrderServiceFault("Es wurde keine bzw. eine ung�ltige W�hrung angegeben!", "");
            }
            ...
            for (ORDERITEM orderItem : order.getORDERITEMLIST().getORDERITEM()) {
                ...
                if (boProduct != null) {
                    ...
                    throw new OrderServiceFault("Die Bestellung kann nicht ausgef�hrt werden! Das Produkt ist nicht vorhanden: " + orderItem.getARTICLEID().getSUPPLIERAID(), "Die Bestellung kann nicht ausgef�hrt werden!");
                }
         ...
         // Write the XML file into the byte array output stream
            ...
            throw new OrderServiceFault("Die Bestellung kann nicht ausgef�hrt werden!", e.getMessage());
